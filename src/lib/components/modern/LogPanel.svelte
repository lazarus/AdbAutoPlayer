<script lang="ts">
  ...
  import { profileLogEntries, clearLogForProfile } from "$lib/taskRuntime";
  ...
  const currentEntries = $derived($profileLogEntries[profileIndex] ?? []);

  // Auto-scroll to bottom on new entries
  $effect(() => {
    const _ = currentEntries.length;
    void _;
    tick().then(() => {
      if (scrollContainer) {
  ...
  function handleClear() {
    clearLogForProfile(profileIndex);
    onClear();
  }
